...
class WebController extends Controller
{
    ...
    protected function checkDashBoardAuth(){
        ...
        $notRequireController = ['AuthController','AjaxController','IndexController'];
...
//            $route = strtolower($controller.'/'.$action); 没必要精确到action
            ...
            if($decoded['role'][0] != '-1'){
                $rule_id = Db::name('admin_rule')->where('name',$route)->value('rule_id');
                if(!in_array($rule_id,$decoded['role'])){
