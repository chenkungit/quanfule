  ...
  <div class="wt">
    <!-- 搜索输入框 -->
       <div class="search">
          ...
          <div class="sou">
              <input placeholder="输入关键字"  @keyup.enter="sou" v-model="keywords"/>
          </div>
      ...
      <div class="itemlist">
          ...
          <ul v-show="isshow" class="animated" :class="{bounceInDown:isshow}">
            <li v-for="(item,i) in paixulist" :class="{active:item.isactive}" :key="i" @click="pxlist(i,item.title,item.order,item.name)">{{item.name}}<i class="iconfont">&#xe604;</i></li>
          </ul>
      ...
      <!-- 列表 -->
       <vscorll :on-refresh="onRefresh" :on-infinite="onInfinite">  
         <div class="goodsbox" :class="isshowstyle!=1?'':'goodstwo'">
            <router-link v-for="item in goodslist" :to="{name:'goodsDetail',query:{gshp_id:item.id}}" :key="item.id">
...
export default {
  ...
  methods: {
    sou() {
      ...
      if (_this.keywords != "") {
         this.getlist(1);
        _this.hc_sou_history.unshift(_this.keywords);
      } else {
        _this.$vux.toast.text("输入不能为空", "middle");
      }
      var arr = utils.unique(_this.hc_sou_history);
      _this.hc_sou_history = arr;
      _this.save_storage("hc_sou_history", arr);
    },
    // 展示列表样式
    showstyle(){
  ...
  a{
    ...
    .txt{
      ...
      p{
        ...
        span{display: inline-block;color: #fff;background: #FF0036;padding:0.03rem 0.1rem;line-height: 0.3rem;margin: 0 0.1rem;font-size: 0.24rem;border-radius: 0.1rem;}
      }
    }
  }
}
