<template>
  <div class="login">
      <header-view title="编辑"></header-view>
      <div class="formbox">
      ...
      <button  type="button" class="cardsave" @click="cardsave">更新</button>
  </div>
</template>
...
export default {
  ...
  methods: {
    cardinfo() {
      var _this = this;
      _this.$http.get(API.cardInfo + this.$route.query.id, {}).then(res => {
    ...
    cardsave() {
      var _this = this;
      _this.$http
        .put(API.cardUpdate + this.$route.query.id, this.userinfo)
        .then(res => {
          if (res.data.code == 200) {
            _this.$vux.toast.text(res.data.msg, "middle");
            setTimeout(function() {
